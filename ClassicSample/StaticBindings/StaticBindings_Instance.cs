/*************************************************************
** Class generated by CodeTrigger, Version 4.8.6.1
** This class was generated on 27.06.2018 13:13:08
** Changes to this file may cause incorrect behaviour and will be lost if the code is regenerated
**************************************************************/
using System;
using System.ComponentModel;
using System.Collections.Generic;
using Awsd5.BusinessObjects;

namespace Awsd5.ClassicSample.StaticBindings
{
	public partial class StaticBindings : INotifyPropertyChanged
	{
		private static StaticBindings _instance = null;
		private static readonly object _lockObj = new object();
		
		private StaticBindings()
		{
			_instance = this;
		}
		
		public static StaticBindings Instance
		{
			get
			{
				lock(_lockObj)
				{
					if(_instance == null)
						_instance = new StaticBindings();
					return _instance;
				}
			}
		}
		
		public event PropertyChangedEventHandler PropertyChanged;
		private void OnPropertyChanged(string propertyName)
		{
			if (this.PropertyChanged != null) 
				this.PropertyChanged(this, new PropertyChangedEventArgs(propertyName));
		}
	}
}
