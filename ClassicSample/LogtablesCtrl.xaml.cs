...
{
	public partial class LogtablesCtrl : UserControl, IRecordList
	{
		...
		private BOLogtables _searchBO;
		private ObservableCollection<BOLogtables> _boLogtabless;
		private IList<BOLogtables> _boLogtablessCached;
		public LogtablesCtrl()
		{
			InitializeComponent();
			resultsBtn.Click += new RoutedEventHandler(resultsBtn_Click);
			updateBtn.Click += new RoutedEventHandler(updateBtn_Click);
			
			_searchBO = new BOLogtables();
			IdTxtBx.DataContext = _searchBO;
			TextTxtBx.DataContext = _searchBO;
			TypeTxtBx.DataContext = _searchBO;
			TimeTxtBx.DataContext = _searchBO;
			UserTxtBx.DataContext = _searchBO;
			
			_boLogtabless = new ObservableCollection<BOLogtables>();
			_boLogtablessCached = new List<BOLogtables>();
			gridView1.ItemsSource = _boLogtabless;
		...
		public void LoadRecords(bool checkLimit, bool showCount)
		{
			...
			try
			{
				...
				if(checkLimit)
				{
					int resultCount = BOLogtables.LogtablesCollectionFromSearchFieldsCount(_searchBO);
				...
				_boLogtablessCached = BOLogtables.LogtablesCollectionFromSearchFields(_searchBO);
				if(checkLimit && (_boLogtablessCached.Count <= _maxResultsCheck) && showCount)
					MessageBox.Show("Your query returned " + _boLogtablessCached.Count + " results.", "Query Results"); 
			
				_boLogtabless.Clear();
				foreach(BOLogtables boLogtables in _boLogtablessCached)
					_boLogtabless.Add(boLogtables);
		...
		private void updateBtn_Click(object sender, EventArgs e)
		{
			...
			{
				Mouse.OverrideCursor = Cursors.Wait;
				foreach(BOLogtables boLogtables in _boLogtabless)
				{
					if(_boLogtablessCached.Contains(boLogtables))
					{
						if(!boLogtables.IsDirty) continue;
						try{ boLogtables.Update(); }
						catch(Exception ex)
				...
				foreach(BOLogtables boLogtables in _boLogtablessCached)
				{
					if(!_boLogtabless.Contains(boLogtables))
					{
						try{ boLogtables.Delete(); }
						catch(Exception ex)
				...
				StaticBindings.StaticBindings.Instance.UpdateLogtablesList();
